...
min_pool = 0 if is_vercel else 10
client = (
    MongoClient(MONGO_URI, tlsCAFile=ca_file, maxPoolSize=200, minPoolSize=min_pool)
    if (MONGO_URI and ca_file)
    else (MongoClient(MONGO_URI, maxPoolSize=200, minPoolSize=min_pool) if MONGO_URI else None)
)
